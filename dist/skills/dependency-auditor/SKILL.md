...
Audit and manage dependencies across multi-language projects. Identifies vulnerabilities, license conflicts, transitive dependency risks, and safe-upgrade paths.

**Source:** claude-skills/engineering/dependency-auditor/SKILL.md (Apache-2.0)

Offline, deterministic dependency auditing across 8+ package ecosystems. The scripts are pattern-matchers over manifests/lockfiles — they do **not** call live advisory APIs; pair findings with `npm audit` / `pip-audit` / `cargo audit` for current CVE coverage.
...
Consume the outputs: `scan.json` findings drive which packages to pin/patch now; `licenses.json` conflicts go to the user as a legal-risk list; `plan.json` orders upgrades by risk with rollback notes.
...
5. **Pair with live advisories**: offline scanning catches patterns; live advisories catch current CVEs — use both
