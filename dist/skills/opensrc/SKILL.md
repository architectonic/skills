...
tags: [agent-operations, dependencies, source-code, debugging, research, npm, pypi, cargo]
...
Fetches dependency source code so agents can read implementations, not just types. Clones repositories at the correct version tag and caches them globally.
...
Source is cached globally (default `~/.opensrc/`, override with `OPENSRC_HOME`).
...
- Pairs with `source-driven-development` — OpenSrc gives you the source; source-driven-development tells you how to use it.
- Pairs with `systematic-debugging` — read library source to understand unexpected behavior.
- Pairs with `api-doc-lookup` — fetch docs first, then source when docs are insufficient.
- Pairs with `codebase-inspection` — understand dependencies as part of codebase analysis.
